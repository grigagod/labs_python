...
def fileGenerator(output_filename, limit, quantity_of_words = (10, 100), length_of_word = (3, 10), progressFlag = False):
    if not (output_filename[-3:] == 'txt' and \
       ...
       (length_of_word[1]-length_of_word[0]) and length_of_word[0]):
        raise ValueError('Incorrect input')
    currentpart = ''
    file = open(output_filename, 'w')
    maximum_of_file = round(limit * 1048576)  # from MB to bytes
    maximum_of_row = (length_of_word[1]) * quantity_of_words[1] # this is done to stop main row-genetator and start generating pseudo random row
...
def lastRowGenerator(length, difference, maxWordLength):
    ...
    while i < (difference - maxWordLength):
         currentWord = lastWordGenerator(maxWordLength)
         i = i + maxWordLength
...
if(__name__ == '__main__'):
    ...
    parser.add_argument("-Q", "--quantity", help="set quantity of words in a row", \
                        default=(10, 100), type=int, nargs=2)
    parser.add_argument("-L", "--length", help="set length of words in a row", \
                        default=(3, 10), type=int, nargs=2)
